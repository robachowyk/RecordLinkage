...
newDirectory = sprintf("NLTCS subset region 1Boston %s", Sys.time())
...
write.csv(df, file.path(newDirectory, "datasetNLTCS_recapnonlinkedFN_Exchanger.csv"))
### PERFORMANCE METRICS
tp <- CT["TRUE", "TRUE"]
fp <- CT["TRUE", "FALSE"]
fn <- CT["FALSE", "TRUE"]
...
write.csv(df, file.path(newDirectory, "datasetNLTCS_recapnonlinkedFN_BRL.csv"))
### PERFORMANCE METRICS
