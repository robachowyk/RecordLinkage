...
simulateD = function(data, D, links, sumRowD, sumColD, truepivsA, truepivsB, gamma, eta, expalpha, phi)
{
  ...
  if ( sum(is.na(Dsample$loglik))>0 | sum(is.na(LLL))>0 | sum(is.na(LLA))>0 | sum(is.na(LLB))>0 | sum(is.na( loglik( LLL=LLL, LLA=LLA, LLB=LLB, D=Dsample$D, links=Dsample$links+1, sumRowD=Dsample$sumRowD, sumColD=Dsample$sumColD, gamma=pLink) ))>0 ){
    print("gamma")
    print(gamma)
    print("links")
    print(links)
...
stEM = function(data, StEMIter, StEMBurnin, GibbsIter, GibbsBurnin, trace=1, UnstableNoMistake=FALSE)
{
  ...
  phi = lapply(data$Nvalues, function(x)  c(0.5,0.1,0.1))
